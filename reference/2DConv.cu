...
constexpr int TILE_IN = BLOCK_DIM + MAX_KS - 1; // 46
constexpr int MAX_KSIZE = MAX_KS * MAX_KS;      // 961
    ...
    int IR, int IC, int kr, int kc, int OR, int OC)
{
    ...
    // 该 block 处理的输出 tile 左上角在 output 坐标
    int out_y0 = blockIdx.y * BLOCK_DIM;
    int out_x0 = blockIdx.x * BLOCK_DIM;

    // 该 block 需要的 input tile 左上角(=输出左上角)
    int in_y0 = out_y0;
    int in_x0 = out_x0;

    int tile_h = BLOCK_DIM + kr - 1; // 实际需要加载的 tile 高
    int tile_w = BLOCK_DIM + kc - 1; // 实际需要加载的 tile 宽

    // ── 1. 加载 input tile 到 shared,所有 256 线程协作 ──
    // tile 最大 46x46 = 2116 个元素,需要循环加载
    ...
    int input_rows, int input_cols, int kernel_rows, int kernel_cols)
{
    ...
    conv2d_kernel<<<grid, block>>>(input, kernel, output,
